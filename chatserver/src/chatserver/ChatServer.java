...
public class ChatServer {
	...
	// Method to broadcast the updated list of client names to all clients
	private synchronized static void broadcastNames() {
	    for (PrintWriter writer : writers) {
	        writer.println("NAMELIST " + names);
	    }
	}
		...
		public void run() {
			try {
				...
				while (true) {
					...
					name = in.readLine();
					System.out.println( name +" was Connected !!");
					for (PrintWriter writer : writers) {							
						writer.println("BROADCAST " + name  +": joined the chat."  );
					}
					if (name == null) {
						return;
					}
...
//				out.println(name);
				...
				// Update all clients with the updated list of names
		        broadcastNames();
				
				//When a name is accepted all the client names are delivered to all the clients 
				for (PrintWriter writer : writers) {	
				...
				while (true) {
					...
					String listner = in.readLine();
					
					//checking if the broadcast is ticked
					//if it is ticked the message is delivered to all the clients with a prefix of broadcast
					if (check.equals("true")) {
						...
						for (PrintWriter writer : writers) {
							writer.println(listner + name +  ": " + input  );
						}
					}
...
//				System.out.println( name + " was Disconnected !!");
		        ...
		        names.remove(name);
		        for (PrintWriter writer : writers) {							
					writer.println("BROADCAST " + name  +": left the chat."  );
				}
			}
			catch (IOException e) {
